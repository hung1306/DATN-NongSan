...
def mean_average_precision(true_labels, pred_scores, k=15):
...
def mean_reciprocal_rank(true_labels, pred_scores, k=15):
    sorted_indices = np.argsort(pred_scores)[::-1][:k]
    true_labels_sorted = np.array(true_labels)[sorted_indices]
...
def ndcg_score(true_labels, pred_scores, k=15):
    sorted_indices = np.argsort(pred_scores)[::-1][:k]
    true_labels_sorted = np.array(true_labels)[sorted_indices]
